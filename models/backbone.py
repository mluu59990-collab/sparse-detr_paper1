...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, return_interm_layers: bool, args):
        ...
        if 'none' in args.backbone:
            ...
            # ✅ THÊM ĐOẠN NÀY: chọn đúng số kênh theo backbone
            if args.backbone in ("resnet18", "resnet34"):
                ch_interm = [128, 256, 512]  # layer2, layer3, layer4
                ch_last = [512]              # layer4
            else:
                ch_interm = [512, 1024, 2048]
                ch_last = [2048]

            if return_interm_layers:
                return_layers = self.get_return_layers(layer_name, (2, 3, 4))
                self.strides = [8, 16, 32]
                self.num_channels = ch_interm
            else:
                return_layers = self.get_return_layers(layer_name, (4,))
                self.strides = [32]
                self.num_channels = ch_last
